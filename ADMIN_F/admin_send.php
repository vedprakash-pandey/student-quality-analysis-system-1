...
$report_sel = "select * from admin_report where admin_sdrn = '$sdrn' and status = 'Not Viewed'";
$report_sel_res  = mysqli_query($conn,$report_sel);
$report_num =  mysqli_num_rows($report_sel_res);

$report_selv = "select * from admin_report where admin_sdrn = '$sdrn' and status = 'Viewed'";
...
$a =array(1);
$report_sel2 = "select report_id from admin_report where admin_sdrn = '$sdrn' and date='$date1'";
...
$report_sel1 = "select * from admin_report where admin_sdrn = '$sdrn' and report_id = '$max_a'";
...
if($date == $date_t){
  ...
  $insert_report = "insert into admin_report(admin_sdrn,date,description,report_id,report) values('$sdrn','$date','$description','$report_id','$report_documents_new')";
...
<body>
    ...
    ?>
            ...
            <div class="form-group">
                    ...
                    <textarea class="form-control" name="description" placeholder="Enter Description..." id="description" rows="5"> </textarea>
            </div>
  ...
  <table class="table-responsive">
        <table class='table table-bordered table-hover'>
                ...
                <?php while($report_row=mysqli_fetch_assoc($report_sel_res)) {?>
                    ...
                    echo "<a href='$report_view' target='_blank' class='btn btn-primary'>View Report</a>";?></td>
                    <td> <?php echo $report_row['status'];?> </td>
                    
                    <td><?php $mark_status = $report_row['status'];
                    ?> <?php if($mark_status == "Viewed"){ echo "disabled"; } ?> 
                                             <?php echo "<a href='delete.php?report=$report_row[report]'><button class='btn btn-danger'>Delete</button></a>";
                                               ?>
                                         </button>
                                         </td>
                </tr>
  ...
  <legend><span class="legend-design">&nbsp; Remarked Reports : &nbsp;</span> </legend>
        ...
        <table class='table table-bordered table-hover'>
                ...
                <?php while($report_rowv=mysqli_fetch_assoc($report_selv_res)) {?>
                    ...
                    <td><textarea style="min-width:100%;" readonly><?php echo $report_rowv['remark'];?></textarea></td>
